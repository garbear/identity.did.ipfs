...
[![Build Status](https://dev.azure.com/teamkodi/binary-addons/_apis/build/status/xbmc.identity.did.ipfs?branchName=Nexus)](https://dev.azure.com/teamkodi/binary-addons/_build/latest?definitionId=50&branchName=Nexus)
[![Build Status](https://jenkins.kodi.tv/view/Addons/job/xbmc/job/identity.did.ipfs/job/Nexus/badge/icon)](https://jenkins.kodi.tv/blue/organizations/jenkins/xbmc%2Fidentity.did.ipfs/branches/)

# InterPlanetary File System (IPFS) identity provider for Kodi

This add-on securely stores identity and credentials on the decentralized InterPlanetary File System (IPFS).
...
make PREFIX=$HOME/kodi ADDONS="identity.did.ipfs"
```
...
mkdir -p identity.did.ipfs
cp -r $HOME/kodi/share/kodi/addons/identity.did.ipfs/ .
cp -r $HOME/kodi/lib/kodi/addons/identity.did.ipfs/ .
```
...
```
make-addons.bat install identity.did.ipfs
```
...
```shell
git clone https://github.com/garbear/identity.did.ipfs.git
cd identity.did.ipfs
mkdir build
cd build
...
```shell
prepare-binary-addons-dev.bat identity.did.ipfs
```
